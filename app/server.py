...
def receive():
    ...
    if args.ledger:
        return_code = ledger.insert(payload)
        if return_code == 501:
...
def query_count():
    ...
    payload = data['payload']
    if args.ledger:
        app.logger.info(f'{payload} was submitted {ledger.get_count(payload)} according to the ledger')
        ...
        if type(payload) is int:
            ...
            app.logger.info(f'{payload} was submitted {results} times since the DB was initialized last.')
